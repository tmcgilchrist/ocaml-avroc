...
let test_boolean _ =
  ...
  check_error
    "Failed to fetch test_double"
    (avro_value_get_boolean !@value_ptr boolean_ptr);
...
let test_string _ =
  ...
  check_error
    ...
    (avro_value_set_string_len !@value_ptr updated (Size_t.of_int smaller_text_len));
  check_error
    "Failed to fetch string"
    (avro_value_get_string !@value_ptr byte_ptr_ptr size_t_ptr);
  let result_str = string_from_ptr !@byte_ptr_ptr ~length:(Size_t.to_int !@size_t_ptr) in
...
let test_array _ =
  ...
  check_error
    ...
    (avro_generic_value_new array_class array_value_ptr);
  let element = allocate_n avro_value ~count:1 in
  let new_index = allocate_n size_t ~count:1 in
  check_error
    "Failed to add element"
    (avro_value_append !@array_value_ptr element new_index);
  check_error
    "Failed to add element"
    (avro_value_append !@array_value_ptr element new_index);
  ...
  let size_t_ptr = allocate_n size_t ~count:1 in
  check_error
    "Failed to get size of array"
    (avro_value_get_size !@array_value_ptr size_t_ptr);
...
let test_enum _ =
  ...
  in
  check_error
    "Failed to read avro schema"
    (avro_schema_from_json_literal schema_str enum_schema_ptr);
...
let test_fixed _ =
  ...
  check_error
    "Failed to read avro schema"
    (avro_schema_from_json_literal schema_str fixed_schema_ptr);
  ...
  check_error
    "Failed to set fixed"
    (avro_value_set_fixed !@value_ptr (to_voidp updated) (Size_t.of_int 4));
...
let test_map _ =
  ...
  check_error
    "Failed to add element"
    (avro_value_add !@map_value_ptr "key1" element new_index is_new);
...
let test_record _ =
  ...
  check_error
    "Failed to read avro schema"
    (avro_schema_from_json_literal schema_str record_schema_ptr);
  ...
  check_error
    "Failed to get field"
    (avro_value_get_by_index !@value_ptr (Size_t.of_int 1) field_ptr field_name_ptr);
  ...
  check_error
    "Failed to get field"
    (avro_value_get_by_name !@value_ptr "i" field_ptr field_index_ptr);
  ...
  check_error
    "Failed to add element"
    (avro_value_append !@field_ptr element_ptr (from_voidp size_t null));
  ...
  check_error
    "Failed to set float"
    (avro_value_set_long !@subfield_ptr (Int64.of_int 10000));
...
let test_union _ =
  ...
  let schema_str =
    "[" ^ "  \"null\"," ^ "  \"int\"," ^ "  \"double\"," ^ "  \"bytes\"" ^ "]"
  in
  check_error
    "Failed to read avro schema"
    (avro_schema_from_json_literal schema_str union_schema_ptr);
  ...
  check_error
    "Failed to get discriminant"
    (avro_value_get_discriminant !@value_ptr discriminant_ptr);
  assert_equal (-1) !@discriminant_ptr;
  let branch_ptr = allocate_n avro_value ~count:1 in
  assert_equal 0 (avro_value_get_current_branch !@value_ptr branch_ptr);
  check_error
    "Failed to get discriminant"
    (avro_value_set_branch !@value_ptr 0 branch_ptr);
  check_error "Failed to set branch" (avro_value_set_null !@branch_ptr);

  check_error
    "Failed to get discriminant"
    (avro_value_set_branch !@value_ptr 1 branch_ptr);
  ...
  check_error
    "Failed to get discriminant"
    (avro_value_set_branch !@value_ptr 2 branch_ptr);
  ...
  check_error
    "Failed to set bytes"
    (avro_value_set_bytes !@branch_ptr (to_voidp bytes) (Size_t.of_int 4));
